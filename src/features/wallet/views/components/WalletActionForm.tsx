...
export function WalletActionForm() {
  ...
  const handleSubmit = (e: React.SyntheticEvent) => {
    ...
    const payload = { amount: value, currency: "USD" };

    if (action === "deposit") {
      depositMutation.mutate(payload, {
        onSuccess: (result) => { toast.success(result.message); setAmount(""); },
  ...
  return (
    <FadeIn delay={150}>
      <div className="rounded-2xl border border-border bg-card p-6">
        <h3 className="mb-6 text-base font-semibold">Add / Withdraw Funds</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isPending}
            className={`flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 font-semibold transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60 ${
              action === "deposit"
                ? "bg-success text-success-foreground"
                : "bg-destructive text-destructive-foreground"
            }`}
